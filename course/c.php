<?php
	...
	if(!isset($_SESSION['id']))
	{
		...
		$s="select uid from opts where uid=".$_SESSION['id']." and cid=6";
		$r=mysqli_query($conn,$s);
		if(!mysqli_num_rows($r))
		{
			$sql="insert into opts values(".$_SESSION['id'].",6,0,'NC')";
...
<p>We guarantee that learning with Learn2Code is fast, effective, and fun. This tutorial will show you how easy it is to become a programmer.</p>
<p>Learn C includes over&nbsp;20&nbsp;lessons that cover&nbsp;<strong>basic concepts, data types, arrays, pointers, conditional statements, loops, functions, structures,&nbsp;unions</strong>, and much more. Unlock new levels, take quizzes, collect points, and compete with peers from around the world.</p>
<p>Most importantly, learn while having fun, because being engaged when learning makes all difference in the world.</p>
  ...
  <div class="courseLessons block">
		...
		$module=0;
		$lesson=0;
		$sq="select count(*) from lesson where toid IN(select tid from topic WHERE coid=6);";
		$resul=mysqli_query($conn,$sq);
		$total=mysqli_fetch_assoc($resul);
		$sql1="select cid from course where cnm='C'";
		...
		if($resultcheck>0)
	  {
	    ...
	    while ($row=mysqli_fetch_assoc($result2))
	    {
				...
				if($resultcheck1>0)
			  {
					$sql4="select last from opts where uid=".$_SESSION['id']." and cid=6";
