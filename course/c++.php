<?php
	...
	$s="select uid from opts where uid=".$_SESSION['id']." and cid=7";
	$r=mysqli_query($conn,$s);
	if(!mysqli_num_rows($r))
	{
		$sql="insert into opts values(".$_SESSION['id'].",7,0,'NC')";
...
<body>
    ...
    <img src="/images/c++.png" alt="C++ Tutorial" class="courseIcon">
    <div class="courseDescription">
        <h1>C++ Tutorial</h1>
        <p>No prior programming experience is necessary! Our C++ tutorial will supply you with everything you need to create and compile your own programs.</p>
<p>We guarantee that learning with Learn2Code is fast, effective, and fun. This app will show you how easy it is to become a programmer.</p>
<p>Learn C++ includes over 20 lessons that cover&nbsp;<strong>basic concepts, data types, arrays, pointers, conditional statements, loops, functions, classes,</strong> and <strong>objects</strong></p>
<p>Most importantly, learn while having fun, because being engaged when learning makes all difference in the world.</p>
    </div>
</div>
...
$lesson=0;
$conn=mysqli_connect('localhost','id13830470_root','Hh6seV>N2Ny?o-q~','id13830470_miniproject');
$sq="select count(*) from lesson where toid IN(select tid from topic WHERE coid=7);";
$resul=mysqli_query($conn,$sq);
$total=mysqli_fetch_assoc($resul);
$sql1="select cid from course where cnm='Cpp'";
...
{$count=0;
	while ($row=mysqli_fetch_assoc($result2))
	{
		...
		if($resultcheck1>0)
		{
			$sql4="select last from opts where uid=".$_SESSION['id']." and cid=7";
			...
			while ($row1=mysqli_fetch_assoc($result3))
			{
				...
				{
					print "<div class=\"courseLesson\">
							<a style=\"pointer-events: none; cursor: default;\" href=\"http://localhost/phplessons/course/lesson.php?id=".$row1['lid']."\">
